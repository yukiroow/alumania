    ...
    <>
      ...
      {!searchQuery ? (
        ...
        <section className="join join-vertical px-[25%] my-10 rounded-box">
          {filter === "users"
            ? results.map((user, index) => <h1 key={index}>{index}: user</h1>)
            : filter === "experiences"
            ? results.map((experience, index) => (
                <h1 key={index}>{index}: experience</h1>
              ))
            : filter === "events"
            ? results.map((event, index) => <h1 key={index}>{index}: event</h1>)
        ...
        <section>
          <div className="block m-auto w-2/5">
            ...
            <p className="text-primary font-thin mb-6">
              Maybe someone&apos;s working on it right now. You never know.
            </p>
          </div>
        </section>
      </div>
